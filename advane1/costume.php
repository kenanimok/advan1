...
<script>
 ...
 <tr>
 <td width="20"><p align ="left" > <a href = "costume.php"><img src = "home.png"
...
<td><p align ="left" id="p_1"><button type="submit" />ค้นหา</button></p></td>
 </tr>
 </table>
 </form>
 <div class="container">
...
if(!$result2) {echo "Can not Show Data"; }
 ...
 <p><a href="order.php?username=<?PHP echo $username; ?>"><font>ประวัติการสั่งซื้อ</font></a></p>
 </form>
 <div class="row">
 <div class="col-sm-4">
 </div>
 ...
 <div class="col-sm-4">
 <p align="right"><img src ="cart.ico" width="30" height="30"> <font id ="a4">( <?PHP
echo $cnt_amount; ?> )</font></p>
 <p align="right"><a href="cart.php?username=<?PHP echo $username ?>&amount=<?PHP echo
$cnt_amount; ?> " ><font id ="a4">ดูสินค้าในตะกร้า</font></a></p>
 </div>
 </div>
 </div>
 <?PHP
 ...
 $sql = "SELECT * FROM cloth where c_name like '%". $c_name ."%' and stock > 0
ORDER by price $type_price";
 }
if($_GET['type_search'] == 'color'){
 $color = $_GET['searching'];
 $sql = "SELECT * FROM cloth where color like '%". $color . "%' and stock > 0
ORDER by price $type_price";
 }
 if($_GET['type_search'] == 'size') {
    $size = strtoupper($_GET['searching']);
    $sql = "SELECT * FROM cloth where size like '$size' and stock > 0 ORDER by price
   $type_price";
    }
   ...
   if($_GET['searching']=="Unisex" or $_GET['searching']=="unisex") {
    $gender = "U";
    $sql = "SELECT * FROM cloth where gender like '$gender' and stock > 0 ORDER
   by price $type_price";
    }
   $sql = "SELECT * FROM cloth where (gender like '$gender' or gender like 'U') and
   stock > 0 ORDER by price $type_price";
    }
    }
   ...
   $sql = "SELECT * FROM cloth Where stock > 0 ORDER by price $type_price";
    ...
    $ct_id = $_GET['category'];
    echo $ct_id;
    $sql = "SELECT * FROM cloth where ct_id like $ct_id and stock > 0 ORDER by price
   $type_price";
    }
    }
    if(!isset($_GET['searching']) and !isset($_GET['ct_id'])) {$sql = "select * from cloth Where
   stock > 0";}
   ...
   <p><font id="a3">Name: </font><font id = "p_1"><?PHP echo $rs['c_name']; ?></font></p>
    ...
    <p><font id="a3">Gender: </font><font id = "p_1"> <?PHP echo $rs['gender'];
   ?></font></p>
    <p><font id="a3">Color:</font> <font id = "p_1"><?PHP echo $rs['color']; ?></font></p>
    <p><font id="a3">Price: </font> <font id = "p_1"><?PHP echo $rs['price']; ?>
   Bath</font></p>
    <p><font id="a3">Stock: </font> <font id = "p_1"><?PHP echo $rs['stock']; ?></font></p>
    ...
    <p><font id="a3">Amount: </font> <font id = "p_1"><input type="number" name="amount"
   placeholder="1" maxlength="3" required min="1" max="100" value="1" /> ea </font></p>
    <p><font id="a3"><button type="submit" id="cart" name="cart" class="btn btnwarning"/>Add to Cart</button></font></p>
    </form>
    </div>
    <?PHP $cnt++;
    }
